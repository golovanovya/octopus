...
const crawl = ( crawlUrl, referenceUrl = '' ) => {
    ...
    return request( crawlUrl, referenceUrl, (requestUrl, pageLinks) => {
        ...
        async.eachSeries( pageLinks, (pageLink, crawlCallback) => {
            ...
            // check max time execution
            const diff = process.hrtime(executionTime);
            const seconds = diff[0];
            const isTimeout = config['max-time'] > 0 && config['max-time'] < seconds;

            // Stream and check next link
            if ( Object.getOwnPropertyNames(nextUrls).length > 1 && !isTimeout) {
            ...
            // Nothing to check, log & exit
                ...
                if (isTimeout) {
                    console.log('breaked by timeout ~%ds', config['max-time']);
                }
...
module.exports = (argv) => {
    ...
    config = {
        ...
        'max-time': Number(argv['max-time'] || 0),
    };
